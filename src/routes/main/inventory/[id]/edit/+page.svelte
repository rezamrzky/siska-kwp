...
<main>
	<section class="place-content-center w-full h-screen drop-shadow-2xl rounded p-5 text-slate-700">
		<div class="w-full max-h-full bg-slate-100 rounded p-5">
			<h2 class="font-['Helvetica Neue'] font-black text-2xl text-primary">Edit Bahan Baku</h2>
			<p>Silahkan mengisi form dibawah:</p>
			<form class="max-h-5/6 overflow-y-scroll" method="POST" use:enhance={submitHandler} action="?/edit">
				...
				<input
					class="input input-bordered w-full bg-slate-100 capitalize disabled:bg-slate-300"
					type="text"
					id="funit"
					bind:value={unit}
                    disabled = {true}
