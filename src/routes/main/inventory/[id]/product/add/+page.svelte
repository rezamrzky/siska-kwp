<script lang="ts">
	...
	function cancelHandler() {
			setDialogue('Batal Tambah Produk?', 'Data yang dimasukkan tidak akan disimpan');
			dialogueOpen.set(true);
			let timeIn = setInterval(() => {
				if (!$dialogueOpen) {
					clearInterval(timeIn);
...
<main>
	<section class="place-content-center w-full h-screen drop-shadow-2xl rounded p-5 text-slate-700">
		<div class="w-full max-h-full bg-slate-100 rounded p-5">
			<h2 class="font-['Helvetica Neue'] font-black text-2xl text-primary">Tambah Produk</h2>
			...
			<form class="max-h-5/6 overflow-y-scroll" method="POST" use:enhance={submitHandler} action="?/add">
				...
				<input class="input input-bordered w-full bg-slate-100 capitalize" type='text' id="fingredients" bind:value={name} name="name"/>
				<label class="label font-bold" for="funit">Satuan</label>
				<input class="input input-bordered w-full bg-slate-100 capitalize" type='text' id="funit" bind:value={unit} name="unit"/>
                <label class="label font-bold" for="funit">Kapasitas</label>
                <div class="flex">
				<input class="input input-bordered bg-slate-100" type='number' id="funit" bind:value={capacity} name="quantity"/>
