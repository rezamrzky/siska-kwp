<script lang="ts">
	...
	$: ({ products } = data);
	let restock = data.restock;
    let restockProduct = data.restockProduct;
	...
	// $: checked = false;
	// $: disabled = !checked;
	let initial_date = formatDateYMD(restock.initial_date),
		latest_date = formatDateYMD(restock.finish_date),
		total = restock.total,
        uraian = restock.note;

	let listOut: Array<number> = [];

        data.products.forEach((obj:any, i:any) => {
            // console.log(obj.id+' '+i)
            if(restockProduct.some((rp:any) => rp.product_id === obj.id && rp.restock_id === restock.id)){
                const rp = restockProduct.find((obj2:any) => obj2.restock_id === restock.id && obj2.product_id === obj.id)
                // console.log(rp.restock_id+' '+rp.product_id+' '+rp.quantity)
                array[i] = rp.quantity
                listOut.push(i)
            }
        })
	...
	function batalHandler() {
		setDialogue('Batal Edit Belanja Dapur?', 'Data yang dimasukkan tidak akan disimpan');
					...
					<tbody class="text-slate-200">
						...
						<label>
							<input type="checkbox" class="checkbox" bind:checked={checked}/>
									...
									<td class="text-center"> {product.brand}, {product.product_unit}</td>
