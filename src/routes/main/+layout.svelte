<script lang="ts">
	// import { username } from '../+page.svelte';
	import { userId } from '$lib/dPegawai';
	import { page } from '$app/stores';

	// import { dialogueOpen, messageOpen } from '../../lib/dialogueStore';
	import { dialogueOpen, messageOpen } from '$lib/dialogueStore';
	...
	import { dUser, getUserByBagian } from '../../lib/dUsers';
	import Dialogue from '../../lib/Dialogue.svelte';
	import Message from '$lib/Message.svelte';

	let user = getUserByBagian(username);
	...
	<section class="navigation basis-1/5 bg-primary h-full drop-shadow-2xl flex flex-col">
		...
		>
			...
			{#if username === '1'}
				...
				<!-- <div class="text-xs ml-4 font-bold">Food & Drink</div> -->
		...
		<footer class="font-['Helvetica Neue] text-sm font-light text-center w-full p-5 text-white">
			<p class="font-bold text-ml">Halo {user?.username}!</p>
			<p>{user?.nama}</p>
			{#if username == '1'}
				<p>Manajer Katering KWP</p>
			{/if}
			{#if username == '2'}
				<p>Admin Food & Drink</p>
			{/if}
			{#if username == '3'}
				<p>Ekonomi PWP</p>
			{/if}
			...
			<button class="btn btn-outline btn-xs"> <a href="/main/user">Ganti Password</a></button>
			<button class="btn btn-outline btn-xs btn-error" on:click={logOutHandler}>Logout</button>
		</footer>
	</section>
