<script lang="ts">
	...
	let tempTotal = 0;
	for (let i = 0; i < data.bill.fd_event.length; i++) {
		// console.log(data.bill.fd_event[i])
		listChecked.push(data.bill.fd_event[i].id);
		tempTotal += +data.bill.fd_event[i].total_price;
	}
	// console.log('tempTotal: ' + tempTotal);

	let totalBeforeTax:number = tempTotal;

	var date: string = formatDateYMD(data.bill.date);
	var desc = data.bill.note;
	...
	const submitHandler: SubmitFunction = ({ cancel }) => {
		...
		return async ({ result }) => {
			if (result.type === 'success') {
				goto('./detail');
				showMessage('Tagihan Berhasil Diubah!');
	...
	function cancelHandler() {
		setDialogue('Batal Edit Tagihan?', 'Data yang dimasukkan tidak akan disimpan');
		dialogueOpen.set(true);
		let timeIn = setInterval(() => {
			if (!$dialogueOpen) {
				clearInterval(timeIn);
...
<main class="mx-auto h-screen flex flex-col bg-slate-100 p-5 ml-2">
	<div>
		<h1 class="font-['Helvetica Neue] w-full text-2xl text-primary font-black">EDIT TAGIHAN</h1>
		<p class="text-slate-700">Silahkan mengubah informasi melalui form dibawah:</p>
	</div>
	<form method="POST" use:enhance={submitHandler} action="?/edit">
				...
				<div class="flex gap-2">
					<div>
						...
						<input
							...
							type="date"
							id="fdateevent"
							min={formatDateYMD(data.bill.created_at)}
							name="due_date"
							bind:value={date}
						/>
					</div>
		...
		<div class="w-full mt-2">
						...
						{#each filteredEvents as event, i}
							{#if event.department.toLowerCase().includes(searchedName.toLowerCase())}
								<tr
									class="text-center"
									on:change={() => {
										onChecked(event.id, event.total_price);
									}}
								>
									<td>
										<input
											...
											checked={listChecked.includes(event.id)}
											name="checked"
											value={event.id}
										/>
									</td>
			...
			<div class="card-actions justify-end p-3">
				...
				{/if}
				<button class="btn" type='button' on:click={cancelHandler}>Batal </button>
